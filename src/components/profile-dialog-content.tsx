...
import { UserRound, UserRoundSearch, Handshake, Pencil, Sun, SunMoon, LaptopMinimal, Loader2 } from "lucide-react"
...
function ProfileDialogContent() {
    ...
    const { mutate: createFriendRequest, state: createFriendRequestState } = useMutationHandler(api.friend_reauest.create)
        ...
        <div>
            ...
            <div className="flex flex-col gap-y-6 mt-4">
                <div className="flex items-center space-x-2 ">
                    <UserRound />
                    <Input disabled placeholder="Name" value={userDetails?.username || "USER NAME"} className="border-none outline-none ring-0" />
                </div>

                <Separator />

                <div className="flex items-center justify-center space-x-5">
                    <p>Manage your accaunt</p>
                ...
                <Dialog open={updateStatusDialog} onOpenChange={() => setUpdateStatusDialog(!updateStatusDialog)}>
                    ...
                    <DialogContent>
                        ...
                        <div className="">
                            {statuses.map(status => (
                                <p key={status} onClick={() => setStatus(status)} className="px-2 py-3 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md cursor-pointer">
                                    {status}
                                </p>
                            ))}
                        </div>
                        <Button onClick={updateStatusHandler} disabled={updateStatusState === "loading"} className="ml-auto w-fit bg-primary-main" type="button">Update status</Button>
                    </DialogContent>
                </Dialog>
